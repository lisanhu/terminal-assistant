...
impl Grid {
    ...
    pub fn visible_rows(&self) -> impl Iterator<Item = &crate::row::Row> {
        ...
        self.scrollback
            ...
            // termassist: backport of the vt100 0.16 fix — when
            // scrollback_offset > rows_len (deep scrollback), cap the
            // number of scrollback rows at rows_len and saturate the
            // subtraction below so deep offsets don't panic.
            .take(rows_len)
            .chain(
                self.rows
                    .iter()
    ...
    pub fn col_wrap(&mut self, width: u16, wrap: bool) {
        if self.pos.col > self.size.cols - width {
            ...
            // termassist: avoid underflow/unwrap panic in degenerate cases
            // (e.g. a 1-row scroll region, where scrolled can exceed
            // prev_pos.row); upstream 0.15.2 and 0.16.2 panic here.
